...
func BuiltinCatalog() *Catalog {
	...
	templates := []core.RouteTemplate{
		...
		{RouteTemplateID: "linux-do-discourse-search", Origin: "builtin", SourceConstraint: core.SourceConstraint{Kind: "exact", Values: []string{"linux.do"}}, Provider: "discourse", Adapter: "discourse_browser", Capabilities: []string{"search"}, ContentLevel: "body", Pagination: core.PaginationDescriptor{Kind: "page"}, TimeRange: core.TimeRangeDescriptor{Kind: "unsupported"}, SearchConstraints: core.SearchConstraintsDescriptor{Time: postFilterTime("day"), Authors: nativeConstraint(), Categories: nativeConstraint(), Tags: nativeConstraint(), ContentFields: nativeConstraint(), Sorts: []core.SearchSort{core.SearchSortRelevance, core.SearchSortNewest}}, Auth: core.AuthDescriptor{Kind: "browser_cookie", Required: true, LoginURL: "https://linux.do/login", Browser: "chrome", PermissionOrigins: []string{"https://linux.do/*"}, CookieScope: &core.CookieScope{URL: "https://linux.do/", AllowedDomains: []string{"linux.do"}, Names: []string{"_t"}, Store: "current", Partitions: []string{"unpartitioned"}}}, EndpointRequired: true, Cost: "rate_limited", Trust: "official_api", Limitations: []string{"chrome_browser_session_required", "discourse_search_date_precision_day"}},
